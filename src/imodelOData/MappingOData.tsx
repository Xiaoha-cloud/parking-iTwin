...
import { Alert, Code, LabeledSelect, SelectOption, Table, ToggleSwitch } from "@itwin/itwinui-react";
...
const fetchMappings = async (
  ...
  const reportingClient = new ReportsClient();
  const accessToken = await IModelApp.authorizationClient!.getAccessToken();
  const reports = await reportingClient.getReports(accessToken, iTwinId);

  if (!reports.length)
    throw new Error("No reports were found");

  const reportMappings = await reportingClient.getReportMappings(accessToken, reports[0].id);
  const labels: SelectOption<string>[] = [];

  for (const reportMapping of reportMappings) {
    const { mapping } = await fetch(reportMapping._links.mapping.href, {
      headers: {
    ...
    labels.push({ value: reportMapping.mappingId, label: mapping.mappingName });
  }

  setMappings(labels);

  if (reportMappings.length > 0) {
    setSelectMapping(reportMappings[0].mappingId);
  }
};
...
export const MappingOData = () => {
  ...
  React.useEffect(() => {
    void fetchMappings(iTwinId, setMappings, setSelectMapping);
  }, [iTwinId]);
